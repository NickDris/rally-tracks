...
REMIND_EVERY_DAYS = int(os.getenv("REMIND_EVERY_DAYS", "7"))
LABEL_NAME = "backport-pending"
...
def gh_req_api(path, method="GET", data=None):
    ...
    if resp.status_code == 403 and resp.headers.get("x-ratelimit-remaining") == "0":
        ...
        time.sleep(wait_sec)
        return gh_req_api(path, method, data)
    if not resp.ok:
...
def gh_req_with_pagination(path, params=None):
    results = []
    page = 1
    params = params or {}
    while True:
        params.update({"per_page": 100, "page": page})
        res = gh_req_api(f"{path}?{'&'.join(f'{k}={v}' for k, v in params.items())}")
...
def list_prs_with_label(label):
    ...
    prs = []
    prs.extend(gh_req_with_pagination(f"/repos/{OWNER}/{REPO}/issues", {"labels": label}))
    return [pr for pr in prs if "pull_request" in pr]


def get_pull(pull_number):
    return gh_req_api(f"/repos/{OWNER}/{REPO}/pulls/{pull_number}")


def list_issue_events(number):
    return gh_req_with_pagination(f"/repos/{OWNER}/{REPO}/issues/{number}/events")


def list_comments(number):
    return gh_req_with_pagination(f"/repos/{OWNER}/{REPO}/issues/{number}/comments")


def create_comment(number, body):
    return gh_req_api(f"/repos/{OWNER}/{REPO}/issues/{number}/comments", method="POST", data={"body": body})
...
def run():
    ...
    print(f"Repeat every: {REMIND_EVERY_DAYS}d")
    ...
    for pr_issue in prs:
        ...
        if age_days < REMIND_EVERY_DAYS:
            print(f"#{number}: label age {age_days}d < {REMIND_EVERY_DAYS}d, skipping.")
            continue
        ...
        if recent_reminder_comments:
            ...
            if since < REMIND_EVERY_DAYS:
                print(f"#{number}: reminded {since}d ago (< {REMIND_EVERY_DAYS}d), skipping.")
                continue
...
This reminder will repeat every `{REMIND_EVERY_DAYS} days` until backport-pending label is removed.
"""
